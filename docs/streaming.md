...
Lookahead processors retain their delay in streamed output. For example, `Limiter#process` emits leading silence equal to `latency`, and `flush` emits the retained final frames. `Stream#write_to` reports this latency but does not remove or shift those samples automatically. Offline `Limiter#apply` uses an isolated runtime and returns a latency-compensated result with the original length, so it does not reset an in-progress `process`/`flush` pass on the same instance.
